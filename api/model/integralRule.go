...
var (
	...
	OPT_CANCEL_ZAN       = "cancelZan"       //被取消赞
)

//初始化数据
var integralRulerInitDataList = []*IntegralRule{
	...
	{TableModel: TableModel{ID: 7}, Operate: OPT_CANCEL_ZAN, Integral: -1, SpeedRate: 1},
}

//积分规则数据初始化
func (*IntegralRule) initIntegralRule(mdb *gorm.DB) {
...
func (m *IntegralRule) BeforeCreate() (err error) {
	m.FlowId = time.Now().Unix()
	return
}

func (m *IntegralRule) Add() error {
	mdb := db.GetMysqlDB()
	tx := mdb.Begin()
	defer tx.Commit()
...
func (m *IntegralRule) Delete(where interface{}, args ...interface{}) error {
	mdb := db.GetMysqlDB()
	tx := mdb.Begin()
	defer tx.Commit()
...
func (m *IntegralRule) Update(update, where interface{}, args ...interface{}) error {
	mdb := db.GetMysqlDB()
	tx := mdb.Begin()
	defer tx.Commit()
...
func (m *IntegralRule) QueryList(field string, page, limit int, where interface{}, args ...interface{}) ([]IntegralRule, error) {
	if 0 > page {
		page = 0
	}
	...
	mdb := db.GetMysqlDB()
	var list = make([]IntegralRule, 0)
	err := mdb.Model(m).Select(field).Where(where, args...).Offset(page * limit).Limit(limit).Find(&list).Error
	if err != nil {
		util.LoggerError(err)
		return make([]IntegralRule, 0), err
	}
	return list, nil
}

func (m *IntegralRule) QueryOne(field string, where interface{}, args ...interface{}) error {
	mdb := db.GetMysqlDB()
...
func (m *IntegralRule) QueryCount(where interface{}, args ...interface{}) (int, error) {
	mdb := db.GetMysqlDB()
	var count int
...
func (m *IntegralRule) QueryGrounp(field string, group string, where interface{}, args ...interface{}) ([]IntegralRule, error) {
	mdb := db.GetMysqlDB()
	var list = make([]IntegralRule, 0)
