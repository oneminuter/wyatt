...
//收藏列表
func (tc *TopicCollectList) List(userId int64) interface{} {
	//查询用户
	var mu model.User
	err := mu.QueryOne("*", "id = ?", userId)
	if err != nil {
	...
	//查询收藏的话题
	var mtc model.TopicCollect
	collects, err := mtc.QueryList("*", tc.Page, tc.Limit, "user_id = ?", userId)
	if err != nil {
		util.LoggerError(err)
	...
	//查询话题信息
	var mt model.Topic
	topics, err := mt.QueryList("*", 0, tc.Limit, "id IN (?)", topicIdArr)
	if err != nil {
		util.LoggerError(err)
		return view.SetErr(constant.QueryDBErr)
	}
